...
my($in,$out,$mut,$workdir,$labeled_muts_file,$plot,$only_extract,@labels,$pc,$quiet,$fcs);
my $debug=0;
$cpu=1;
...
my $data=Extract::quick($debug>1 ? 1 : 0);
...
warn "\nRunning predictions..\n" unless $quiet;
my @predictions=Predict::all($data,"$snap2dir/quick",$debug>1 ? 1 : 0);
